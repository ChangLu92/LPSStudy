---
title: "All figures of LPS study"
...
# please download the LINCS CMap L1000 data GSE 92742 and GSE70138 based on this link:https://clue.io/connectopedia/lincs_cmap_data, and save them to drugfilepath
...
target_list = subvertexdf %>% filter(hubgenes == 'yes' )
...
# # this will take a long time. The results are saved in 'results/druglistCSSalmonhub.csv', so you can also just read this csv file
# cl <- makeCluster(5)
...
# write.csv(df,file = 'data/druglistCSSalmonhub.csv', row.names = FALSE)


df = read.csv('data/druglistCSSalmonhub.csv')
# this is the table for visualizing Fig 4
finallist.filter = df %>% mutate(p.Adj = p.adjust(df$PValue, method = "fdr")) %>%
  filter(p.Adj<0.05,pert_iname !='DMSO',Connectivity>0.5) %>% 
